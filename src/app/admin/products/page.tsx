...
import { Plus, Search, Filter, MoreVertical, Edit2, Trash2, Tag, Copy, Loader2, ExternalLink, ArrowUpDown, ArrowUp, ArrowDown, Upload, FileSpreadsheet, Download, CheckCircle2, XCircle } from 'lucide-react';
...
export default function ProductsManagement() {
    ...
    const [importResult, setImportResult] = useState<{ imported: number; failed: number; errors: { row: number; field: string; message: string }[] } | null>(null);
    ...
    const handleCsvImport = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (data.failed > 0) {
                    toast.error(`${data.failed} rows failed`);
                }
            } else {
                setImportResult(data);
        ...
        <div className="flex flex-col gap-8">
            ...
            {isImportModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 antialiased">
                    ...
                    <div className="bg-slate-900 border border-slate-800 w-full max-w-lg rounded-2xl shadow-2xl z-10 overflow-hidden relative flex flex-col">
                        ...
                        <div className="p-6 flex flex-col gap-5">
                            ...
                            <a
                                ...
                                download
                                className="flex items-center gap-2 text-sm text-primary hover:text-sky-400 font-bold transition-colors"
                            >
                            ...
                            <div
                                className={`border-2 border-dashed rounded-xl p-8 text-center transition-colors ${
                                    importFile ? 'border-primary bg-primary/5' : 'border-slate-700 hover:border-slate-600'
                                }`}
                            ...
                            >
                                {importFile ? (
                                    <div className="flex items-center justify-center gap-3">
                                        ...
                                        <button onClick={() => { setImportFile(null); setImportResult(null); }} className="ml-4 text-slate-400 hover:text-red-400">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm font-bold text-slate-300">Drag & drop a CSV file here</p>
                            ...
                            <div className="text-xs text-slate-500 space-y-1">
                                <p><span className="font-bold text-slate-400">Required:</span> name, price</p>
                                <p><span className="font-bold text-slate-400">Optional:</span> category, form, description, image, stockStatus, variations</p>
                                <p><span className="font-bold text-slate-400">Variations format:</span> {`[{"option":"5mg","price":49.99}]`}</p>
                            </div>
                        </div>
                        <div className="p-6 border-t border-slate-800 flex justify-end gap-3">
                            <button onClick={() => setIsImportModalOpen(false)} className="px-5 py-2.5 rounded-xl font-bold text-slate-300 bg-slate-800 hover:bg-slate-700 transition-colors">Cancel</button>
                            <button
                                onClick={handleCsvImport}
                                disabled={!importFile || importLoading}
                                className="px-5 py-2.5 rounded-xl font-bold text-white bg-primary hover:bg-sky-500 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                            >
